#!/usr/bin/env node
// CI byte-identical reproducibility check for java.wasm.
...
const committed = await readFile(path.join(repoRoot, "java.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed java.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-java-verify-"));
...
console.log(`rebuilt java.wasm sha256: ${rebuiltHash}`);
