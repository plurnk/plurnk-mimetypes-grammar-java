#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-java.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-java-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-java.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as java.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "java.wasm"));
const bytes = (await readFile(path.join(repoRoot, "java.wasm"))).length;
console.log(`java.wasm: ${bytes} bytes (built from ${pin})`);
